    ...
    async execute(interaction) {
        if (interaction.isButton()) {
            if (interaction.customId == "run_broadcast_button") {
                ...
                if (!thetokens) return interaction.reply({ content: `**لم يتم اضافة اي توكن لبوتات البرودكاست**`, ephemeral: true });
                if (thetokens.length <= 0) return interaction.reply({ content: `**لم يتم اضافة اي توكن لبوتات البرودكاست**`, ephemeral: true });
                const broadcast_msg = db.get(`broadcast_msg`)
